...
module Amaze::Script::AsciiOptions
  ...
  def parser
    ...
    opts.on('--ascii-grid-color NAME', Rainbow::X11ColorNames::NAMES.keys, 'The color of the grid.') do |color|
      ascii_options[:grid_color] = color
    end
    opts.on('--ascii-path-color NAME', Rainbow::X11ColorNames::NAMES.keys, 'The color of the path, when drawing the solution or longest path.') do |color|
      ascii_options[:path_color] = color
    end
    opts.on('--ascii-distances-color NAME', Rainbow::X11ColorNames::NAMES.keys, 'The color of the distances.') do |color|
